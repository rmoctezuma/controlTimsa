          ...
          foreach ($nwerows as $fila) {
            $nombreSocio = "correcto";
              if($fila['status'] == 'Libre'){
                ...
                $Contador += 1;
                  $contenido .= '<button type="button" val="'.$fila['idSocio'].'" title="'.$fila['Placas'].'" class="btn btn-large economicosButtons" data-toggle="button">'.$fila['Economico']. '</button>';
              }
              else{
                if($Contador == 3) {$contenido .= '<br>';}
                $Contador += 1;
                 $contenido .= '<button type="button" val="'.$fila['idSocio'].'" title="'.$fila['Placas'].'" class="btn btn-large disabled economicosButtons ocupado" disabled = "disabled" data-toggle="button">'.$fila['Economico']. '</button>';
              }
          }
